define [
	...
	Backbone.View.extend
		...
		render: (el) ->
			...
			$("#app-view-controller-" + @number + " .app-content-container").html(@$el)

			setTimeout( =>
				@loadMainWindow(@mainView)
			, 5)
			@updateTopbarTitle()
		open: (type, options, number) ->
			...
			if @type isnt "im"
				...
				@currentUser = swipy.collections.users.findWhere({name: @identifier})
		...
		updateTopbarTitle: ->
			...
			swipy["topbar" + @number].setTitle(name)
		loadMainWindow: (type) ->
			@vc?.destroy()
			@vc = @getChatListVC()
			@$el.html @vc.el
